...
class MainColour {
    ...
    colour2: string;
    colour3: string[];
    constructor(public gen_color1, public gen_color2, public gen_color3) {
...
function processImage(callback) : void {
    ...
    if (filesize > 4) { //check the input file size
...
function sendColorRequest(file) : void {
    $.ajax({
        async: false,
...
function sendTagRequest(file) : void {
    $.ajax({
